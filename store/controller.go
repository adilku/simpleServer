...
// Page number nth GET
func (c *Controller) GetPage(w http.ResponseWriter, r *http.Request) {
    log.Println(r.URL.Query().Get("page"))
    number := r.URL.Query().Get("page") //3 param query
    log.Println("GetPage number - " + number);
    num_int, err := strconv.Atoi(number)
    if err != nil {
        fmt.Println("error in strconv")
        return
    }


    sort_type := r.URL.Query().Get("s")
    products := c.Repository.GetProductsByPage(num_int, sort_type)
    data, _ := json.Marshal(products)
    w.Header().Set("Content-Type", "application/json; charset=UTF-8")
    w.Header().Set("Access-Control-Allow-Origin", "*")
    w.WriteHeader(http.StatusOK)
